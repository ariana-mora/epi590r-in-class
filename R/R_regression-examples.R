...
# one predictor across several outcomes. Create a univariate regression table
...
#if all were the same regression model, change outcome
# tbl_uvregression(
# 	nlsy,
# 	x = sex_cat,
...
tbl_merge(list(m1, m2, m3, m4),
					tab_spanner = c("**Number of Siblings**", "**Sleep (weekday)**", "**Sleep (weekend)**", "**Income**"))
...
# Since family = binomial(link = "log") often doesn’t converge,
# we often use Poisson regression with robust standard errors to estimate risk ratios.
# Fit a Poisson regression instead of the log-binomial regression in the last question.
# Then create a table using tidy_fun = partial(tidy_robust, vcov = "HC1").
